...
const obtenerNivelesDeSatisfaccion = () => {
    ajax({
        url: "http://192.168.0.101:4040/encuesta",
        method: "GET",
        success:(respuesta) => {
            console.log(respuesta);
            respuesta.forEach(encuestas_registradas => {
                if(encuestas_registradas.nivel_satisfaccion == 3){
                    numero_encuestas_felices += 1;
                }else if (encuestas_registradas.nivel_satisfaccion == 2){
                    numero_encuestas_tristes += 1;
                }else if (encuestas_registradas.nivel_satisfaccion == 1){  
